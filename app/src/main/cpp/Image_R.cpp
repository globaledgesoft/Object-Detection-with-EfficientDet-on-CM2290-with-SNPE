...
void OnImageCallback(void *ctx, AImageReader *reader) {
  reinterpret_cast<Image_R *>(ctx)->ImageCallback(reader);
}


Image_R::Image_R(ImageFormat *res, enum AIMAGE_FORMATS format)
    : reader_(nullptr),
      presentRotation_(0),
...
void Image_R::ImageCallback(AImageReader *reader) {
  int32_t format;
...
ANativeWindow *Image_R::GetNativeWindow(void) {
...
AImage *Image_R::GetNextImage(void) {
  AImage *image;
...
AImage *Image_R::GetLatestImage(void) {
  AImage *image;
 ...
 */
int32_t Image_R::GetMaxImage(void) {
  int32_t image_count;
...
void Image_R::DeleteImage(AImage *image) {
  if (image != nullptr) {
    AImage_delete(image);
    image = nullptr;
  }
...
bool Image_R::DisplayImage(ANativeWindow_Buffer *buf, AImage *image) {
...
void Image_R::PresentImage(ANativeWindow_Buffer *buf, AImage *image) {
...
void Image_R::PresentImage90(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
...
void Image_R::PresentImage180(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
...
void Image_R::PresentImage270(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
...
void Image_R::SetPresentRotation(int32_t angle) {
