...
std::map<std::string, std::vector<float>> Qcsnpe::predict(cv::Mat input_image) {
    ...
    for(auto i =0 ; i<input_image.rows;i++) {
        for(auto j =0 ;j<input_image.cols;j++ ) {
            for(auto k =0 ;k<input_image.channels();k++) {
                tensor_ptr_fl[i*input_image.cols*input_image.channels() +j*input_image.channels()+k] = static_cast<float>(input_image.data[i*input_image.cols*input_image.channels() +j*input_image.channels()+k]) ;
                tensor_ptr_fl[i*input_image.cols*input_image.channels() +j*input_image.channels()+k] /=255.0;
                tensor_ptr_fl[i*input_image.cols*input_image.channels() +j*input_image.channels()+k] -= mean[k];
                tensor_ptr_fl[i*input_image.cols*input_image.channels() +j*input_image.channels()+k] /= std[k];
            }
        }
    }
    //infer
