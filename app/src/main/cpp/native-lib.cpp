...
Java_com_object_recognition_MainActivity_onCreateJNI(JNIEnv *env, jobject clazz, jobject activity, jobject j_asset_manager) {
...
Java_com_object_recognition_MainActivity_setSurface(
        JNIEnv* env, jobject clazz,
        jobject surface) {

    app.SetNativeWindow(ANativeWindow_fromSurface(env, surface));
    app.SetUpCamera();
    std::thread loopThread(&main::Cameraiter, &app);
    loopThread.detach();
}
